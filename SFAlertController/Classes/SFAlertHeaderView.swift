...
internal class SFAlertHeaderView: UIView {
    /// 内容视图
...
extension SFAlertHeaderView {
    func config(using title: String?, message: String?, customView: UIView?, width: CGFloat) {
        ...
        if customView.frame != CGRect.zero {
            ...
            let customViewSize = customView.systemLayoutSizeFitting(
                CGSize(width: customViewWidth, height: CGFloat.greatestFiniteMagnitude),
